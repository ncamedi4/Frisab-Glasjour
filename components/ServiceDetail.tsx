...
export const ServiceDetail: React.FC<Props> = ({ serviceId, onBack }) => {
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="relative h-[240px] md:h-[450px] overflow-hidden">
        <img src={data.heroImage} alt={data.title} className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
          <div className="container mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-6xl font-semibold text-white uppercase tracking-tighter mb-4">{data.title}</h1>
            <div className="w-16 md:w-20 h-1.5 md:h-2 bg-frisab-blue mx-auto"></div>
          </div>
        </div>
        <button 
          onClick={onBack}
          className="absolute top-6 left-4 md:top-10 md:left-12 bg-black/30 hover:bg-black/50 text-white px-4 md:px-6 py-2.5 md:py-3 rounded-sm backdrop-blur-md flex items-center gap-2 transition-all font-semibold text-[10px] md:text-xs uppercase tracking-widest active:scale-95"
        >
          <ArrowLeft size={16} className="md:w-5 md:h-5" /> Tillbaka
        </button>
      </div>
      ...
      <div className="container mx-auto px-4 py-12 md:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 md:gap-16">
          <div className="lg:col-span-2 space-y-8 md:space-y-12">
            ...
            <p className="text-lg md:text-2xl text-gray-600 leading-relaxed font-normal italic">{data.description}</p>
            
            <div className="bg-zinc-50 p-6 md:p-10 border border-zinc-100 rounded-lg">
              <h3 className="text-[10px] md:text-xs font-semibold text-frisab-dark uppercase mb-6 md:mb-8 border-b-2 border-frisab-blue/20 pb-3 tracking-[0.2em]">Våra tjänster inkluderar:</h3>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                {data.points.map((point, i) => (
                  <li key={i} className="flex gap-3 md:gap-4 items-start">
                    ...
                    <span className="text-gray-700 font-normal text-sm md:text-lg">{point}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="prose max-w-none">
              <p className="text-gray-600 text-base md:text-xl leading-relaxed whitespace-pre-line font-normal">{data.detailedText}</p>
            </div>

            {data.extraNote && (
              <div className="bg-frisab-yellow/10 border-l-[6px] md:border-l-8 border-frisab-yellow p-6 md:p-8 flex gap-4 md:gap-6 items-center rounded-r-lg">
                <AlertCircle className="text-frisab-dark shrink-0" size={24} />
                <p className="text-[10px] md:text-xs font-semibold uppercase text-frisab-dark tracking-widest leading-relaxed">{data.extraNote}</p>
              </div>
            )}
          </div>

          <div className="space-y-8">
            <div className="bg-zinc-900 text-white p-8 md:p-10 border-l-[6px] md:border-l-8 border-frisab-yellow shadow-2xl lg:sticky lg:top-40 rounded-r-lg">
              <h3 className="text-xl md:text-2xl font-semibold mb-4 md:mb-6 uppercase leading-none italic text-frisab-yellow">Boka service</h3>
              ...
              <div className="space-y-3 md:space-y-4">
                <a href="tel:0708955030" className="flex flex-col items-center justify-center bg-frisab-blue text-white py-4 md:py-5 font-semibold text-[10px] md:text-xs uppercase tracking-widest hover:bg-blue-700 transition-colors active:scale-[0.98]">
                  <span className="opacity-50 text-[9px] md:text-[10px] mb-1 font-normal">Tommy</span>
                  0708-95 50 30
                </a>
                <a href="tel:0708955025" className="flex flex-col items-center justify-center bg-white text-black py-4 md:py-5 font-semibold text-[10px] md:text-xs uppercase tracking-widest hover:bg-zinc-100 transition-colors border-2 border-transparent active:scale-[0.98]">
                  <span className="opacity-50 text-[9px] md:text-[10px] mb-1 font-normal">Jonny (Jour)</span>
                  0708-95 50 25
                </a>
              </div>
            </div>
